...
// 💡 ASUME: Aquí ya tienes tu import { proyectsData } from "../data/index"
// O si estás usando props, ajusta el inicio de la función:
import { proyectsData } from "../data/index" 

const ProjectPage = ({ projects = proyectsData }) => {
  return (
      ...
      {projects.map ((data, index) => (
        // 💡 CLAVE: Usa un key único para View Transitions
        <article key={data.title} className='flex gap-2 rounded-2xl m-8 overflow-hidden lg:pr-0 lg:pl-0 flex-col-reverse xl:flex-row lg:max-w-6xl xl:h-96 border-3 justify-between items-center hover:scale-103 transition duration-200 ease-out'>
      ...
      <a className="text-[20px] size-fit bg-purple-700 py-2 px-5 rounded-[10px] transition active:scale-93 font-semibold" href="/">Volver</a>
    </section>
  );
}
